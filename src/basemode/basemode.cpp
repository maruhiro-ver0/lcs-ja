   ...
   while(true)
   {
      ...
      if(!forcewait)
      {
         ...
         if(sieged)
         {
            ...
            mvaddstr(19,1,"F - 脱出/戦闘");

            set_color(COLOR_WHITE,COLOR_BLACK,0);
            mvaddstr(19,23,"G - 投降する");
         }
         else
         {
            if(partysize) set_color(COLOR_WHITE,COLOR_BLACK,0);
            else set_color(COLOR_BLACK,COLOR_BLACK,1);
            mvaddstr(19,1,"F - 悪を阻止するために出撃する");
         }
         ...
         else
         {
            ...
            else mvaddstr(23,1,"W - 一日を過ごす");
            if(day==monthday()) addstr(" (月替わり)");
         }

         set_color(COLOR_WHITE,COLOR_BLACK,0);
         mvaddstr(22,40,"S - 言論の自由: リベラルスローガン");
         if(haveflag)
         {
            if(sieged) set_color(COLOR_GREEN,COLOR_BLACK,1);
            else set_color(COLOR_WHITE,COLOR_BLACK,0);
            mvaddstr(22,1,"P - 抗議: 国旗を焼き払う");
         }
         else
         {
